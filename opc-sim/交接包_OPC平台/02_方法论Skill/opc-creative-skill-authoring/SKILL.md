...
21. **截断批次按产物矩阵恢复**：并行批次遇到503/524/iteration cap时，不整体重跑。先逐目标检查文件矩阵；完整项回读关键字段和真实执行证据后标PASS，缺失项缩成“单目录/单文件/一至三镜”重派。代理摘要中的`completed`不覆盖文件不存在或测试缺失事实。
...
故事板采用按关键画面状态动态决定宫格数的多宫格模式，不固定 4 格或 9 格。每格必须对应一个可检查的状态；如果关键状态过多，应先检查镜头是否超载并拆镜。

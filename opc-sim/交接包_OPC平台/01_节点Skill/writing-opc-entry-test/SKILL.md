...
## Core Contract
1. **画风前置硬门禁**：平台下拉选择的 `style_id` 必须在写作启动前存在；未选择画风时返回 `STYLE_REQUIRED`，禁止写作。
2. **唯一开放入口**：运行时唯一外部创意来源是本次客户提交 `customer_idea` 及同一载荷中的明确约束。
3. **一次提交**：客户提交次数为 1；写作过程中不提问、不等待确认、不接收外部创意补料。
4. **唯一首交**：完成时只对外交付一部完整小说；内部草稿、备选路径和失败稿不得外显。
5. **下游封闭**：输出完成后，下游只能使用本版本小说、随附记录及已验收的结构化上游产物。
6. **事实不越权**：客户显式事实和明确限制不可静默改写；缺项只能保守补齐并留下记录。
7. **结果可判定**：最终只能是 `completed` 或 `blocked`，两者互斥。

## Character Appearance Baseline

Unless the customer or story explicitly defines an exception such as an ugly comic
role, disfigurement, disguise, illness, or aging state, characters use the OPC high-
attractiveness baseline. This changes visual casting only, not personality, morality,
class, or dramatic function. Record facial structure, eye spacing/shape, nose, lips,
jaw, hairline, body proportions, and clothing as visible identity facts; do not rely on
the words beautiful or handsome alone. See `references/opc-high-attractiveness-character-rules-v0.1.md`.

## Input Boundary

最小输入是一个非空、具有小说语义的 `customer_idea`。接受自然语言、结构化对象、数组或混合形式。
同时必须有平台下拉选择的 `style_id`；没有选择不得使用默认画风代替。
...
未选择画风、空输入、无小说语义单元，或无法在不违反锁定事实的情况下消解核心冲突时，进入对应阻断状态。

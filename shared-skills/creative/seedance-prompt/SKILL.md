...
description: "This skill should be used when the user asks to write, improve, translate, compress, or debug a Seedance 2.0 video prompt; mentions T2V, I2V, V2V, R2V, camera direction, prompt quality, or provides reference assets for a production-ready prompt.（中文触发：写、改、翻译、压缩Seedance提示词时加载）"
license: MIT
user-invocable: true
tags:
  - prompt-engineering
...
Load `[参考:quick-ref未安装扩展，以本正文为准]` for the checklist, `[参考:reference-workflow未安装扩展，以本正文为准]` for multimodal references, `[参考:i2v-guide未安装扩展，以本正文为准]` for image-to-video, `[参考:first-last-frame-guide未安装扩展，以本正文为准]` for first/last-frame work, `[参考:examples-by-mode未安装扩展，以本正文为准]` when examples are useful, `[参考:shot-list-continuity未安装扩展，以本正文为准]` for multi-shot professional plans, `[参考:multishot-grammar未安装扩展，以本正文为准]` for shot-label grammar, the shots-times-seconds budget, and cut placement inside one generation, and `[参考:multilingual-community-examples未安装扩展，以本正文为准]` for Chinese/Russian/Japanese/Korean/Spanish or mixed-language prompts. When sequence state is present, load `[参考:prompt-compiler未安装扩展，以本正文为准]` and compile only the current clip contract.

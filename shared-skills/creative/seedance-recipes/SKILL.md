...
description: "This skill should be used when the user asks for a Seedance 2.0 template, genre recipe, product ad, lifestyle video, drama scene, music video, landscape shot, commercial, animation scene, or reusable production pattern.（中文触发：要Seedance模板、类型片配方、现成提示词时加载）"
license: MIT
user-invocable: true
tags:
  - templates
...
Load `[参考:genre-guides未安装扩展，以本正文为准]` for genre patterns, `[参考:examples-by-mode未安装扩展，以本正文为准]` when the user needs copy-ready examples, `[参考:shot-list-continuity未安装扩展，以本正文为准]` for professional multi-shot sequences or commercials, and `[参考:multilingual-community-examples未安装扩展，以本正文为准]` when the recipe should reflect Chinese/Russian/Japanese/Korean/Spanish community-style structures.

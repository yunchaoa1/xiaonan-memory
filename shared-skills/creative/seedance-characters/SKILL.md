...
description: "This skill should be used when the user asks for character consistency, character tags, identity lock, multi-character blocking, wardrobe continuity, hand safety, expression control, or likeness-sensitive character guidance.（中文触发：Seedance角色一致性、角色绑定、人物外观保持时加载）"
license: MIT
user-invocable: true
tags:
  - characters
...
Load `[参考:shot-list-continuity未安装扩展，以本正文为准]` when character identity, wardrobe, props, eyeline, screen direction, or emotional state must survive across multiple shots. Load `[参考:directing-engine未安装扩展，以本正文为准]` to direct performance: give each character a playable objective, show subtext through contradiction between word and action, and keep one performance register consistent with the project's directorial voice.

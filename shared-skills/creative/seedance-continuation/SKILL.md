...
description: "This skill should be used when a Seedance 2.0 user asks to continue, extend, make the next part, repair the tail, bridge between known frames, re-anchor drift, or create a successor prompt from accepted footage.（中文触发：Seedance续集、接镜、延续上一段剧情时加载）"
license: MIT
user-invocable: true
tags:
  - continuation
...
This skill body is the continuation workflow. Project state and prompt-compilation rules live in the seedance-prompt and seedance-sequence skill bodies; load seedance-troubleshoot when the continuation failed or drift is visible. The next clip inherits the project's directorial voice from seedance-director; the look never re-rolls between clips.

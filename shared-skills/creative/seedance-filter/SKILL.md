...
description: "This skill should be used when a Seedance 2.0 prompt is blocked, rejected, silently degraded, or likely to trigger a content filter; or when the user asks for a safer rewrite without losing the creative intent.（中文触发：Seedance生成被拦截、拒审、静默失败时加载）"
license: MIT
user-invocable: true
tags:
  - content-filter
...
Load `[参考:filter-vocab未安装扩展，以本正文为准]` for safer substitutions. Load `[参考:multilingual-community-examples未安装扩展，以本正文为准]` only when the safe repair needs Chinese/Russian/Japanese/Korean/Spanish or mixed-language wording for clarity.

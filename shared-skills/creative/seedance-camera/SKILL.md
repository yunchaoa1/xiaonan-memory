...
description: "This skill should be used when the user asks for camera movement, shot scale, lens feel, framing, one-take direction, dolly, pan, tilt, push-in, handheld, aerial, macro, or camera-transfer guidance for Seedance 2.0.（中文触发：Seedance镜头方案、运镜、景别、镜头语言时加载）"
license: MIT
user-invocable: true
tags:
  - camera
...
Load `[参考:quick-ref未安装扩展，以本正文为准]` for prompt assembly, `[参考:cinematography-shot-language未安装扩展，以本正文为准]` for professional shot contracts, `[参考:directing-engine未安装扩展，以本正文为准]` to derive the move from the scene's one intention so it reinforces light, performance, and sound instead of competing, and `[参考:vocab/zh未安装扩展，以本正文为准]` or `[参考:vocab/ru未安装扩展，以本正文为准]` when camera wording must be multilingual.

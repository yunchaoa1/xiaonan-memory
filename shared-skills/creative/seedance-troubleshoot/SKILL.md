...
description: "This skill should be used when a Seedance 2.0 output is blurry, jittery, off-prompt, morphing, blocked, visually generic, unstable, desynced, inconsistent, or otherwise fails and needs root-cause diagnosis.（中文触发：Seedance输出模糊、抖动、人物崩、偏离需求时加载）"
license: MIT
user-invocable: true
tags:
  - diagnostics
...
If the take is partially good rather than failed, route to `[参考:retake-protocol未安装扩展，以本正文为准]` triage first - most takes deserve a verdict, not a rewrite. Diagnose failure before rewriting. Do not simply add more adjectives. Identify whether the failure came from mode mismatch, overload, ambiguity, fragile identity, unsafe wording, unsupported platform behavior, or missing preservation constraints.

When the diagnostic tree has no row for the failure, load `[参考:model-mechanics未安装扩展，以本正文为准]` and diagnose by mechanism: attention dilution, prior conflict, summoned negation, broken trajectory, compounding error, conditioning conflict, capacity starvation, or an overloaded audio-video joint constraint. Load `[参考:field-observed-tips未安装扩展，以本正文为准]`, `[参考:reference-workflow未安装扩展，以本正文为准]`, and `[参考:api-workflow未安装扩展，以本正文为准]` when the failure involves continuation, edit/extend, source clips, audio references, or platform-specific errors. Load `[参考:shot-list-continuity未安装扩展，以本正文为准]` for multi-shot drift and `[参考:delivery-qc未安装扩展，以本正文为准]` for final-client or delivery failures. When sequence state is present, load `[参考:failure-atlas未安装扩展，以本正文为准]`, `[参考:continuation-handoff未安装扩展，以本正文为准]`, and `[参考:continuity-qc未安装扩展，以本正文为准]`; diagnose against continuity locks, completed beats, exact reference tags, and reserved future beats.

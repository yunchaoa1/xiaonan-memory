---
name: seedance-audio
description: "This skill should be used when the user asks for Seedance 2.0 audio, dialogue, lip-sync, music, sound effects, ambience, beat-sync, audio-reference mapping, desync troubleshooting, or sound-driven visual timing.（中文触发：Seedance音频、对白、音效、口型同步相关需求时加载）"
license: MIT
user-invocable: true
tags:
  - audio
...
Load `[参考:audio-guide未安装扩展，以本正文为准]` for how the audio model behaves, per-language dialogue capacity, the voice-reference lip-sync path, beat-sync, desync repair, audio-reference conflicts, and multi-character workarounds. Load `[参考:audio-post-delivery未安装扩展，以本正文为准]` when the user needs stems, M&E, dubbing, loudness, sync, mix, or delivery guidance.
...
Reliability is probabilistic and language-dependent: field reports rank Mandarin strongest for lip-sync, English a close second, with Japanese, Korean, Russian, and others weaker. Keep non-English lines very short or use a voice reference, and budget retakes rather than promising a clean voiced take. See `[参考:audio-guide未安装扩展，以本正文为准]` for the field-observed per-language dialogue-capacity table.

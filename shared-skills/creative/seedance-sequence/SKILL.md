...
description: "This skill should be used when a Seedance 2.0 request is a long story, connected set of clips, multi-generation scene, campaign sequence, dense storyboard, continuation-ready plan, or any idea that must be divided into stateful clips.（中文触发：Seedance长故事、多段连续剧情、多镜串联时加载）"
license: MIT
user-invocable: true
tags:
  - sequence
...
Load `[参考:sequence-project-state未安装扩展，以本正文为准]`, `[参考:continuation-handoff未安装扩展，以本正文为准]`, `[参考:prompt-compiler未安装扩展，以本正文为准]`, `[参考:surface-prompt-profiles未安装扩展，以本正文为准]`, `[参考:event-density未安装扩展，以本正文为准]`, and `[参考:continuity-qc未安装扩展，以本正文为准]`. Load `[参考:reference-transfer-contract未安装扩展，以本正文为准]` when references are present and `[参考:dense-storyboard-mode未安装扩展，以本正文为准]` when the request contains many shots or animation panels. Load `[参考:directing-engine未安装扩展，以本正文为准]` to set one directorial voice for the whole story and plan the long-form spine so the look is authored by one hand across every clip. For a user's first multi-clip project, `[参考:sequence-worked-trace未安装扩展，以本正文为准]` walks the whole loop once - plan, deviation, reconciliation, chain cap, re-anchor, resume.

...
## Closed Runtime Information Boundary（2026-08-31锁定）

OPC 一键生产节点采用封闭信息边界。运行时唯一合法信息源为：

1. 原始小说及明确版本；
2. 流程内上游 Skill 已验收的结构化产物；
3. 当前节点 Skill 已锁定的规则、默认值和输出契约。

运行时禁止采纳聊天补充、人工场外解释、网络资料、临时口头设定、其他项目资料和未验收草稿。不得将这些内容静默并入成品。缺必要信息时，使用已验证默认值；无法可靠补全时，输出缺项或阻断状态，不向用户提问补齐。

外部资料只允许用于 Skill 制作与验证阶段，建立方法证据和测试规则；不得成为生产节点运行时的隐式输入。

...
- 主包：`D:\Documents\我的文档\十二时辰_第一章_剧本资产提取_Skill测试版v0.9.md`
- 逐镜CSV：`D:\Documents\我的文档\十二时辰_第一章_逐镜资产绑定_v0.9.csv`
- 当前没有子代理运行。
- v0.9尚未重新独立验收，不得宣称通过。
...
1. 完成资产提取v0.9修复与独立PASS。
2. 创建/测试GPT Image主体资产提示词/生成Skill。
3. 生成并锁定第一章人物、场景、道具主体图。
4. 分镜绑定锁定主体资产。
5. 使用主体图生成动态宫格故事板。
6. 拆关键帧/首尾帧。
7. 创建/测试MiniMax H3全能参考提示词Skill。
8. 再处理创作者候选证据补强；它不是当前生产链阻断项。
